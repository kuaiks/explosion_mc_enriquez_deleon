...
from dominio.modelos import ConfigExplosion # para definir la configuración de la simulación.
from dominio.puertos import PuertoEntrada # para cumplir con la de entrada que el dominio espera.
from adaptadores.entrada.yaml_config import cargar_config_yaml # para cargar valores por defecto desde un archivo YAML.


class EntradaCLI(PuertoEntrada): # se encarga de leer la configuración desde la línea de comandos y convertirla en ConfigExplosion para que el dominio pueda usar.

    def obtener_config(self) -> ConfigExplosion: # lee argumentos de la terminal y los convierte en una instancia de ConfigExplosion.
        ...
        ) # este formateador hace que al usar --help se muestren los valores por defecto que definimos en el YAML.
